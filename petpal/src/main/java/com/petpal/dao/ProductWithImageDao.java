...
public class ProductWithImageDao {
	...
	RowMapper<ProductWithImageDto> mapper = new RowMapper<ProductWithImageDto>() {
		...
		public ProductWithImageDto mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			return ProductWithImageDto.builder().productNo(rs.getInt("product_no"))
					.productName(rs.getString("product_name")).categoryCode(rs.getString("category_code"))
					.productPrice(rs.getInt("product_price")).productStock(rs.getInt("product_stock"))
					.productDesc(rs.getString("product_desc")).productRegdate(rs.getDate("product_regdate"))
					.productDiscount(rs.getInt("product_discount")).productViews(rs.getInt("product_views"))
	...
	RowMapper<ProductWithImageDto> parentMapper = new RowMapper<ProductWithImageDto>() {
		...
		public ProductWithImageDto mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			return ProductWithImageDto.builder().productNo(rs.getInt("product_no"))
					...
					.productDesc(rs.getString("product_desc")).productRegdate(rs.getDate("product_regdate"))
					.productDiscount(rs.getInt("product_discount")).productViews(rs.getInt("product_views"))
					.attachmentNo(rs.getInt("attachment_no")).categoryParent(rs.getString("category_parent")).build();
		}
	...
	public List<ProductWithImageDto> selectList1(PaginationVO vo, String column, String keyword) {
		String sql = "select * from(" + "select rownum rn, TMP.* from("
				+ "select * from product where instr(#1,?)>0 order by product_regdate desc" + " )TMP"
