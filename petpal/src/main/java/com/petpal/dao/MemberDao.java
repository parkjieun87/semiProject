...
public class MemberDao {
	...
	// 비밀번호 제외한 나머지 정보 변경 기능 구현(형석)
		...
		public void certifiedPhoneNumber(String userPhoneNumber, int randomNumber) {
			String api_key = "NCS2MIOLDQAQDKUM";
		    String api_secret = "L9BVTDO86DNWMOLVZNOHKM7CB9WHFLIA";
		    Message coolsms = new Message(api_key, api_secret);

		    // 4 params(to, from, type, text) are mandatory. must be filled
		    HashMap<String, String> params = new HashMap<String, String>();
		    params.put("to", "01088191375");    // 수신전화번호
		    params.put("from", "01088191375");    // 발신전화번호. 테스트시에는 발신,수신 둘다 본인 번호로 하면 됨
		    params.put("type", "SMS");
